...
def plot_result(agent_parameters, directory, max_steps):
            ...
            for update_ss in agent_parameters["update_step_size"]:
                for avg_reward_ss in agent_parameters["avg_reward_step_size"]:
                    for epsilon in agent_parameters["epsilon"]:
                        load_name = 'semi-gradient_sarsa_tilings_{}_tiledim_{}_update_ss_{}_epsilon_ss_{}_avg_reward_ss_{}_max_steps_300000'.format(
                        ...
                        graph_current_data, = ax[0].plot(plt_x_legend, data_mean, linewidth=1.0,
                                                         label="update_ss: {}/32, epsilon: {}, avg reward step_size: {}".format(
                                                             update_ss, epsilon, avg_reward_ss))
                        ...
                        graph_current_data, = ax[1].plot(plt_x_legend, data_mean, linewidth=1.0,
                                                         label="update_ss: {}/32, epsilon: {}, avg reward: {}".format(
                                                             update_ss, epsilon, avg_reward_ss))
    ...
    plt.suptitle("Average Reward Semi-gradient Sarsa ({} Runs)".format(len(data)), fontsize=16, fontweight='bold',
                 y=1.03)
    ...
    plt.savefig('sarsa_specific.png')
    plt.show()


def plot_sweep_result(directory, max_steps):
    ...
    for setting in top_results:
        ...
        load_name = 'semi-gradient_sarsa_tilings_{}_tiledim_{}_update_ss_{}_epsilon_ss_{}_avg_reward_ss_{}_max_steps_{}'.format(num_tilings, num_tiles, update_ss, epsilon, avg_reward_ss, max_steps)
        file_type2 = "exp_avg_reward"
        data = np.load('{}/{}_{}.npy'.format(directory, load_name, file_type2))
