                    ...
                    for epsilon in agent_parameters["epsilon"]:
                        ...
                        for run in tqdm(range(1, experiment_parameters["num_runs"] + 1)):
                            ...
                            while num_steps < experiment_parameters['max_steps']:
                                num_steps += 1
                                if experiment_parameters['max_steps'] - num_steps==10000:
...
def A_2D_mash_grid():
    ...
    agent_parameters = {
        ...
        "update_step_size": [7],
        "epsilon": [0.05],
        "avg_reward_step_size": [2**-4],
        "num_actions": 3,
        "iht_size": 4096*13
    }
    #avg 2^-9, 2^-8, 2^-7, 2^-6, 2^-5, 2^-4
    #num_runs 300000, 200000, 100000, 50000, 20000
    #max_i = 10
    #max_j = 1
    epislon_range = np.array([2**-11, 2**-10, 2**-9, 2**-8, 2**-7, 2**-6, 2**-5, 2**-4, 2**-3, 2**-2])
    update_step_size_range = np.array([2**-10, 2**-9, 2**-8, 2**-7, 2**-6, 2**-5, 2**-4, 2**-3, 2**-2, 2**-1])
    result_exp_error = np.empty([10,10]) 
    ...
    temp = unravel_index(result_exp_error.argmax(), result_exp_error.shape)
    print(epislon_range[temp[0]],update_step_size_range[temp[1]])
    #np.savetext("50000_run_avg_step_size_0.07_32_tilings_num_10_10_epsilon_0.005_0.02_update_step_size_2_-2_2_5.txt", result_exp_error)
...
#    [0.00097656 0.00642361 0.01187066 0.01731771 0.02276476 0.02821181
#     0.03365885 0.0391059  0.04455295 0.05]
    # np.linspace(2**-10, 0.05, num=10)
    epislon_range = np.array([0.00097656, 0.00642361, 0.01187066, 0.01731771,0.02276476,0.02821181,0.03365885,0.0391059,0.04455295,0.05])
    #print(epislon_range)
    result_exp_error = []
    test_scope = int(0.25*max_steps) # we test the exponential return on the last 50000 time steps.
    current_env = PendulumEnvironment
    current_agent = SarsaAgent
    file_type = "exp_avg_reward"
    directory = "results_sarsa"
    ...
    figure(figsize=(8,6))
    plt.plot(epislon_range,np.array(result_exp_error)+0.01)
    plt.xticks(epislon_range, fontsize = 14)
    plt.tight_layout()
    plt.xlabel("Epsilon", fontsize = 18)
    plt.ylabel("Exponential Average Reward for the Last 5000 Time Steps", fontsize = 16)
    plt.title("Effect of Changing Epsilon", fontsize = 18)
    plt.tight_layout()
    plt.savefig('sarsa_epsilon_test_300000.png')
...
def test_parameters_update_step_size():
    ...
    update_step_size_range = np.linspace(2**-9, 2**0, num=10)
    result_exp_error = []
    current_env = PendulumEnvironment
    current_agent = SarsaAgent
    test_scope = int(0.25*max_steps) # we test the exponential return on the last 50000 time steps.
    ...
    plt.tight_layout()
    plt.savefig('sarsa_upate_step_size_test_300000.png')
    plt.clf() # clear the old plot
...
def test_parameters_avg_step_size():
    ...
    plt.plot(avg_reward_step_size_range, np.array(result_exp_error)+0.09)
    ...
    plt.tight_layout()
    plt.savefig('sarsa_avg_reward_step_size_test_300000.png')
