            ...
            for update_ss in agent_parameters["update_step_size"]:
                for avg_reward_ss in agent_parameters["avg_reward_step_size"]:
                    for epsilon in agent_parameters["epsilon"]:
                        ...
                        save_name = "semi-gradient_sarsa_tilings_{}_tiledim_{}_update_ss_{}_epsilon_ss_{}_avg_reward_ss_{}".format(
                            num_tilings, num_tiles, update_ss, epsilon, avg_reward_ss)
...
def get_data(agent_parameters, directory, file_type):
    ...
    load_name = 'semi-gradient_sarsa_tilings_{}_tiledim_{}_update_ss_{}_epsilon_ss_{}_avg_reward_ss_{}'.format(
        num_tilings, num_tiles, update_ss, epsilon, avg_reward_ss)
...
def A_2D_mash_grid():
    ...
    agent_parameters = {
        ...
        "update_step_size": [7],
        "epsilon": [0.05],
        "avg_reward_step_size": [0.11],
        "num_actions": 3,
        "iht_size": 4096*20
    }
    max_i = 16
    max_j = 16
    epislon_range = np.linspace(0.005, 0.02, num=max_i)
    update_step_size_range = np.linspace(2**-1, 2**5, num=max_j)
    ...
    np.savetxt("20000_run_32_tilings_avg_step_0.11_update_0.5_32.txt", result_exp_error)
    fig = plt.figure()
...
def test_parameters_update_step_size():
    ...
    update_step_size_range = np.linspace(2**-2, 2**4, num=10)
    result_exp_error = []
    current_env = PendulumEnvironment
    current_agent = SarsaAgent
    test_scope = 2000 # we test the exponential return on the last 50000 time steps.
    ...
    plt.tight_layout()
    plt.savefig('sarsa_upate_step_size_test.png')
    plt.clf() # clear the old plot
...
def test_parameters_avg_step_size():
    ...
    avg_reward_step_size_range = np.linspace(0.001, 0.05, num=10)
    result_exp_error = []
    test_scope = 2000  # we test the exponential return on the last 50000 time steps.
    current_env = PendulumEnvironment
    current_agent = SarsaAgent
    ...
    plt.tight_layout()
    plt.savefig('sarsa_avg_reward_step_size_test.png')
