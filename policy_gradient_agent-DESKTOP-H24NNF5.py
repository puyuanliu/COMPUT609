...
def run_specific():
    ...
    num_tilings_range = [[4], [8], [16], [32], [64], [90]]
    # Agent parameters
    ...
    for num_tilings in num_tilings_range:
        agent_parameters["num_tilings"] = num_tilings
        run_experiment(current_env, current_agent, environment_parameters, agent_parameters, experiment_parameters)
    plot_script.plot_sweep_result('results_actor_critic')
...
def test_parameters():
    ...
    result_exp_error = []
    test_scope = 2000 # we test the exponential return on the last 50000 time steps.
    current_env = PendulumEnvironment
    current_agent = ActorCriticSoftmaxAgent
    ...
    print(actor_step_size_range.round(3))
    print([round(num, 3) for num in result_exp_error])
    plt.xticks(actor_step_size_range, fontsize = 14)
    plt.xlabel("32 * Actor Step Size", fontsize = 18)
    plt.ylabel("Exponential Average Reward for the Last 5000 Time Steps", fontsize = 14)
   ...
   # print(critic_step_size_range.round(3))
    #print([round(num, 3) for num in result_exp_error])
    plt.xticks(critic_step_size_range,  fontsize = 14)
    plt.xlabel("32 * Critic Step Size", fontsize = 18)
    plt.ylabel("Exponential Average Reward for the Last 5000 Time Steps", fontsize = 14)
    plt.title("Effect of Changing Critic Step Size", fontsize = 18)
    plt.tight_layout()
    #plt.savefig('critic_step_size_test_20000.png')
    ...
    plt.plot(avg_reward_step_size_range.round(3), result_exp_error)
    print(avg_reward_step_size_range.round(3))
    print([round(num, 3) for num in result_exp_error])
...
def A_2D_mash_grid():
    ...
    np.savetext("20000_run_avg_reward_step_size_2^-5-_policy_gradient.txt", result_exp_error)
